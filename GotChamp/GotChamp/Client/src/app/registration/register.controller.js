...
angular.module('gotChamp')
  .factory('RegisterFactory',['$http','authSetting',function($http,authSetting){
    ...
    .factory('ConfirmationFactory', ['$resource', 'authSetting', function ($resource, authSetting) {
        return $resource(authSetting.serviceBaseUrl + '/api/account/confirmation', {userId:'@userId',code:'@code'}, { confirm: { method: 'POST' } });
    }])
   .controller('RegisterCtrl', ['$scope', '$rootScope', '$cookies', '$location', '$routeParams', 'toastr', 'RegisterFactory', 'ConfirmationFactory', 'localStorageService', 'LocationsFactory', 'PrefectureFactory',
       function ($scope, $rootScope, $cookies, $location, $routeParams, toastr, RegisterFactory,ConfirmationFactory, localStorageService, LocationsFactory, PrefectureFactory) {
       ...
       ConfirmationFactory.confirm({ userId: userId, code: $.urlParam('code') }, function (data) {
