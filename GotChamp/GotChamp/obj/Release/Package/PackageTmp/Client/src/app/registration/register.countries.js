...
//        if(option_str.options. == "Japan") {
//            
//        }
//		option_str.options[option_str.length] = new Option(country_arr[i], country_arr[i], true);
//	}
    
    var japan = 0;
    
    var container = document.createElement("option");
    container.text = "Select Country"
    
    for(var x = 0; x < country_arr.length; x++) {
        if(country_arr[x] == "Japan") {
            japan = x;
        }
        
        var data = document.createElement("option");
        data.text = country_arr[x];
        option_str.options.add(data, x);
    }
    option_str.options.add(container, 0);
    option_str.selectedIndex = japan;
    
    option_str.options[0].disabled = true;
    
    print_state('state', option_str.selectedIndex + 1);
//    alert(option_str.selectedIndex);
}
...
function print_state(state_id, state_index) {
	...
	if (state_index == 115) {
	    option_str.options[0] = new Option('Select Prefecture', '');
	}
	else {
